...
class Assembly(BaseModel):
    ...
    def to_cq(self, project: "Project", abs_location: Optional[Location] = None):
        ...
        for subassembly_id in self.children:
            subassembly = project.assemblies[subassembly_id]
            asm_abs_location = Location.convert(subassembly.location).transform(
                abs_location
            )
            cq_assembly.add(
                subassembly.to_cq(project),
...
class CadService:
    ...
    def read_cq_assembly(
        ...
        project.assemblies[root_assembly.id] = root_assembly
        ...
        for cq_subassembly in cq_assembly.children:
            if isinstance(cq_subassembly, cq.Assembly) and len(cq_subassembly.children):
                ...
                is_modified = not (
                    ...
                    index.prev_project
                    and part_ref.id in index.prev_project.part_refs
                    and
                    # if the part checksum is the same
                    index.prev_project.part_refs[part_ref.id].variation
                    == part_ref.variation
                )
                ...
                project.part_refs[part_ref.id] = part_ref
                ...
                existing_variation = project.inventory.get_variation_from_color(
                    part_ref.variation.checksum, part_color
                )

                if part_checksum not in project.inventory.catalog.items:
                    project.inventory.catalog.items[part_checksum] = CatalogItem(
                ...
                if not existing_variation:
                    ...
                    project.inventory.catalog.items[part_checksum].variations.append(
                        part_variation
                    )
                else:
                ...
                if index and index.prev_project:
                    prev_variation = (
                        index.prev_project.inventory.get_variation_from_color(
                            part_ref.variation.checksum, part_color
                        )
                    )
    ...
    def assign_unique_part_names(
        part_ref: PartRef, project: Project, index: "AssemblyIndex"
    ):
        ...
        if part_name in index.part_names:
            ...
            if prev_part_ref and part_ref.variation != prev_part_ref.variation:
                ...
                prev_part_name = "-".join(
                    prev_path_strs[
                        min(project.options.max_name_depth, len(prev_path_strs)) :
                    ]
                )
                assert (
                    prev_part_name not in index.part_names
                ), f"part name {prev_part_name} already exists"
                project.inventory.catalog.items[
                ...
                part_name = "-".join(
                    path_strs[min(project.options.max_name_depth, len(path_strs)) :]
                )
                assert (
                    part_name not in index.part_names
                ), f"part name {part_name} already exists"
                project.inventory.catalog.items[part_ref.variation.checksum].name = (
                    part_name
                )
    ...
    def get_part(
        ...
        is_part_reference = (options and options.use_part_references) and (
    ...
    def get_non_reference_part(
        ...
        if normalize_axis:
            ...
            if index.prev_project and aligned_checksum in index.aligned_refs:
                ...
                base_part = index.prev_project.inventory.parts[
    ...
    @staticmethod
    def get_inventory_markdown(
        inventory: Inventory, project_path: Union[str, Path, None] = None
    ):
        md = "# Inventory\n"
        data = []
        for catalog_item in inventory.catalog.items.values():
            ...
            if project_path:
                ...
                svg_path = (
                    project_path / f"./assets/{catalog_item.name}.svg"
            ...
            for variation in catalog_item.variations:
                ...
                data_item = {
                    ...
                    "Price": (
                        f"${variation.metadata.price}"
                        if variation.metadata and variation.metadata.price
                        else "-"
                    ),
                    "URL": (
                        variation.metadata.url
                        if variation.metadata and variation.metadata.url
                        else "-"
                    ),
                }
                if svg_path and variation.id == 1:
                    data_item["Part"] = (
                        f"![{catalog_item.name}-{variation.id}](../{svg_path})"
                    )
                if variation.id > 1:
                    data_item["Name"] = "\n"
    ...
    @staticmethod
    def write_inventory(
        project_path: Union[Path, str], inventory: Inventory, verbose=False
    ):
    ...
    def write_assets(
        ...
        with open(inventory_path / "README.md", "w") as f:
            inventory_md = CadService.get_inventory_markdown(
                project.inventory, assets_path
            )
        ...
        if (
            not index
            or index
            and project.root_assembly.id in index.is_assembly_modified
            or not root_assembly_svg_path.exists()
        ):
            logger.info(
                f"- Generating SVG for root assembly '{project.root_assembly.name}', this may take a sec ..."
            )
    ...
    def write_assemblies(
        project_path: Union[Path, str], project: Project, verbose=False
    ):
        logger.setLevel(logging.INFO if verbose else logging.ERROR)
    ...
    def revise_project(
        ...
        if write:
            CadService.write_project(
                project_path, revised_project, index, verbose=verbose
            )
