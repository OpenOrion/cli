...
from orion_cli.models.archive import Inventory
...
class AssetHelper:
    ...
    def get_inventory_markdown(
        inventory: Inventory, archive_path: Union[str, Path, None] = None
    ):
        md = "# Inventory\n"
        data = []
        for catalog_item in inventory.catalog.items.values():
            ...
            if archive_path:
                ...
                svg_path = (
                    archive_path / f"./assets/{catalog_item.name}.svg"
                ).relative_to(archive_path)
